...
public abstract class Camera1Base
  ...
  public boolean prepareVideo(int width, int height, int fps, int bitrate, int rotation) {
    return prepareVideo(width, height, fps, bitrate, 5, rotation);
  }
  ...
  public boolean prepareVideo() {
    ...
    return prepareVideo(960, 540, 20, 1800 * 1024, 0);
  }
   ...
   *
   * @return true if success, false if you get a error (Normally because the encoder selected
   * doesn't support any configuration seated or your device hasn't a AAC encoder).  8000, 16000, 22500, 32000, 44100.
   */
  ...
  public void startPreview(CameraHelper.Facing cameraFacing) {
    ...
    startPreview(cameraFacing, 1920, 1080);
  }
